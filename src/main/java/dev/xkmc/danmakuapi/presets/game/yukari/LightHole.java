package dev.xkmc.danmakuapi.presets.game.yukari;
...
public class LightHole extends ActualSpellCard {
	...
	public static class LaserAdder extends Ticker<LightHole> {
		...
		public boolean tick(CardHolder holder, LightHole card) {
			...
			var r = holder.random();
			if (tick == 0) {
				holder.shoot(holder.prepareLaser(100, pos, ori.rotateDegrees(-45), 40, DanmakuItems.Laser.LASER, DyeColor.RED));
				holder.shoot(holder.prepareLaser(100, pos, ori.rotateDegrees(45), 40, DanmakuItems.Laser.LASER, DyeColor.BLUE));
			}
		...
		private void addLaserBeams(CardHolder holder, Vec3 pos, Vec3 dir, double step, double r, DyeColor color) {
			...
			holder.shoot(holder.prepareLaser(100, p, ori, 40, DanmakuItems.Laser.LASER, color));
		}
